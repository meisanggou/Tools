...
from JYTools.JYWorker import RedisWorker, TaskStatus, receive_argv

__author__ = 'meisanggou'


class PipelineWorker(RedisWorker):
    ...
    def analysis_ref(self, ref_str, current_index, task_len):
        match_r = re.match("^(\\d{1,10})([a-z]\\w{0,20})$", ref_str)
        if match_r is None:
...
if __name__ == "__main__":
    ...
    app = PipelineWorker(conf_path=conf_path, heartbeat_value=heartbeat_value, work_tag=work_tag, log_dir=log_dir)
